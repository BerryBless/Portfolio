...
// 3. 등록된 오브젝트의 맴버함수에서 OBJECT_MOVE(x,y) 호출, this 오브젝트가 [x][y] 좌표로 이동시 true, 실패시 OnCollision 호출후 false 반환
...
#define OBJECT_MOVE(x,y)	I_FRAMEWORK->TryMove(x,y,this)				// 이 오브젝트가 [x][y] 좌표로 이동시 true, 실패시 OnCollision 호출후 false 반환
...
// Collision(pos, pObj)	: pObj의 목적지 pos에 오브젝트(pTarget)가 존재하면 pObj->OnCollision(pTarget) 호출하고 true를 리턴 
// Find(type);			: 이타입의 오브젝트중 _objects 에서 첫번째로 마주치는 오브젝트 반환 못찾으면 nullptr 반환
// TryMove( pos, pObj)	: mobj를 pos로 이동 시도. 실패했고, 실패한이유가 target 오브젝트와 부딭친거면 mobj->OnCollision(target) 호출
...
public:
	...
	// obj의 목적지 pos에 오브젝트(pTarget)가 존재하면 obj->OnCollision(pTarget) 호출하고 true를 리턴 
	bool Collision(int iX, int iY, IObject *obj);
	bool Collision(Position pos, IObject *obj);	
